...
void lcd_set_window(uint16_t xstar, uint16_t ystar, uint16_t xend, uint16_t yend)
{
    ...
    databuf[0] = xstar >> 8;   /* 高八位 */
    databuf[1] = 0xFF & xstar; /* 底八位 */
    databuf[2] = xend >> 8;    /* 高八位 */
    databuf[3] = 0xFF & xend;  /* 低八位 */
...
void lcd_show_char(uint16_t x, uint16_t y, uint8_t chr, uint8_t size, uint8_t mode, uint16_t color)
{
    ...
    if (size != 24)
    {
        ...
        for (t = 0; t < csize; t++)
        {
            temp = pfont[t]; /* 获取字符的点阵数据 */
                             /**
                              * 1. 在前面已经通过命令的方式圈定了显示区域
                              * 2. 通过循环将每个字节的数据写入到LCD，每次循环写入一个像素点，每个bit位代表一个像素点
                              * 3. 1206的数据，每行有6个像素点。每个字节数据只用到了6位，所以只需要循环6次
                              * 4. 1608的数据，每行有8个像素点。每个字节数据只用到了8位，所以只需要循环8次
                              * 5. 3216的数据，每行有16个像素点。每两个字节表示一行，因为已经设置了显示区域，
                              *    写完一个像素点后，会自动从上到下，从左到右移动地址。
                              * */
            for (t1 = 0; t1 < sta; t1++)
            {
                if (temp & 0x80)
    ...
    else
    {
        /**
         * 2412需要特殊处理
         * 1. 2412数据，每一行有12个像素点，但也需要用到两个字节存储数据，其中第二个字节只用到前四位
         * 2. 所有需要判断当前字节序号，如果是偶数，就用8位，如果是奇数，就用4位
         **/
        csize = (size * 16) / 8;
...
void lcd_init(void)
{
    ...
    lcd_init_cmd_t ili_init_cmds[] =
        {
            {0x11, {0}, 0x80}, /* 退出睡眠模式，0x11直接就是寄存器的值没有数据 */
            {0x36, {0x00}, 1}, /* 设置显示方向从左到右从上到下 */
            {0x3A, {0x65}, 1}, /* 0x65: 65h = 0b01100101 设置颜色格式*/
            {0X21, {0}, 0x80}, /* 开显示反转 */
            {0x29, {0}, 0x80}, /* 显示开 */
            {0, {0}, 0xff},
        };
